...
apt-get install -yq \
    ...
    traceroute \
    iperf3 \
    apache2-utils # to htpasswd to generate HTTP basic auth files
...
cat <<'EOF' > /etc/ssh/sshd_config.d/99-ux-key-security.conf
Protocol 2
PermitRootLogin no
...
cat <<'EOF' > /etc/ssh/sshd_config.d/99-ux-key-display.conf
PrintLastLog no
EOF

systemctl restart ssh

ufw logging off # To avoid this flood https://serverfault.com/questions/1152709/why-im-getting-these-random-ufw-block-requests-in-my-syslogs
...
echo "$SERVER_FQDN installation completed" 
